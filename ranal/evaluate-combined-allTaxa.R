...
files = list.files('../result/output_aggregated_pivoted', full.names=TRUE, pattern='*specie_pivoted.csv')
print(files)
...
df.res.draw %>%
  ...
  # geom_text(aes(label=predict.counts), position=position_dodge(width=10), vjust=-0.25)


ggsave("fig/combined-prediction-accuracy-by-network-types-allTaxa.png", height=8, width=8)
ggsave("fig/combined-prediction-accuracy-by-network-types-allTaxa.pdf", height=8, width=8)
...
# geom_text(aes(label=predict.counts), position=position_dodge(width=10), vjust=-0.25)
ggsave("fig/combined-prediction-accuracy-allTaxa.png", height=8, width=8)
ggsave("fig/combined-prediction-accuracy-allTaxa.pdf", height=8, width=8)
...
print(
  ...
  file = "fig/combined-results-summarize-table-allTaxa.tex",
