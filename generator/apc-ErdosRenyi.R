...
generate.er.nm = function(n, m=round(n*n*.8)) {  
  sample_gnm(n, m, directed = T) %>% generate.asymmetric.G
}
...
get.Sigma.chiquet = function(n, u=0.3, v=0.1) {
  inv.Omega = generate.er.nm(n) %>% get.Omega.chiquet(u, v) %>% inv
...
sample.squencing = function(n.species, 
                            ...
                            u=0.3, v=0.1,
                            m.edges = round(n.species^2 *.8)
                            ) {
  G = generate.er.nm(n.species, m.edges)
